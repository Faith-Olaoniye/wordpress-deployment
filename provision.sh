...
# ===========================================
# provision.sh
# Sets up an EC2 Ubuntu server to run
# the WordPress + MySQL Docker application
# ===========================================

# ---- Step 1: Update system packages ----
# Always update first so we install the latest
# and most secure versions of everything
...
# We check if Docker is already installed before
# trying to install it again. This makes the script
...
# Docker Compose lets us run multiple containers
...
# The AWS CLI is needed by backup.sh to upload
...
# This is the folder where the EBS volume will be
# mounted. MySQL data will be stored here so it
...
# We check if it is already mounted before mounting.
# Mounting twice causes errors. The EBS volume should
# appear as /dev/nvme1n1 after being attached in AWS.
...
# Without this, the EBS volume unmounts every time
# the server restarts. Adding it to /etc/fstab makes
...
# We give full ownership to ubuntu and open permissions
# so Docker can read and write freely.
